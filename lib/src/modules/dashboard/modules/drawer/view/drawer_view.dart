...
class DrawerView extends GetView<DashboardDrawerController> {
  ...
  Widget _buildMenuItem(BuildContext context, DrawerMenuModel item) {
    if (item.isExpansion && item.subItems != null) {
      return ExpansionTile(
        leading: _buildIcon(context, item, false),
        title: Text(item.title, style: context.titleStyleRegular.copyWith(fontWeight: FontWeight.w600)),
        children: item.subItems!.map((subItem) => _buildSubMenuItem(context, subItem)).toList(),
      );
    }

    // Reintegrating Active State Highlighting for Single Page Application
    return Obx(() {
      final dashboardController = Get.find<DashboardController>();

      // Mapped to the correct 'selectedMenuKey' property in your DashboardController
      final isActive = dashboardController.selectedMenuKey.value == item.identifier;
      ...
      return Container(
        ...
        child: ListTile(
          ...
          title: Text(
            item.title,
            style: isActive
                ? context.titleStyleRegular.copyWith(color: Theme.of(context).primaryColor, fontWeight: FontWeight.bold)
                : context.titleStyleRegular,
          ),
          onTap: () {
            dashboardController.changeActiveMenu(item.identifier);
            // Dynamic Platform Awareness: Auto-close overlay on Mobile
            if (Scaffold.of(context).hasDrawer && Scaffold.of(context).isDrawerOpen) {
              Get.back();
            }
          },
  ...
  Widget _buildSubMenuItem(BuildContext context, DrawerMenuModel item) {
    return Obx(() {
      ...
      return Container(
        ...
        child: ListTile(
          contentPadding: EdgeInsets.only(left: context.responsiveWidth(56, 56), right: context.responsiveWidth(16, 16)),
          leading: _buildIcon(context, item, isActive, size: context.responsiveSize(20, 20)),
          title: Text(
            item.title,
            style: isActive
                ? context.bodyTextStyle.copyWith(color: Theme.of(context).primaryColor, fontWeight: FontWeight.bold)
                : context.bodyTextStyle,
          ),
          onTap: () {
            dashboardController.changeActiveMenu(item.identifier);
            if (Scaffold.of(context).hasDrawer && Scaffold.of(context).isDrawerOpen) {
  ...
  Widget _buildIcon(BuildContext context, DrawerMenuModel item, bool isActive, {double? size}) {
    final double iconSize = size ?? context.responsiveSize(24, 24);
    final Color iconColor = isActive ? Theme.of(context).primaryColor : Colors.grey[700]!;
    ...
    if (item.icon != null && item.icon!.isNotEmpty) {
      return SizedBox(
        ...
        child: Image.network(
          item.icon!,
          fit: BoxFit.cover,
          color: isActive ? Theme.of(context).primaryColor : null, // Tint the image if active
          errorBuilder: (context, error, stackTrace) {
            Sentry.addBreadcrumb(
              Breadcrumb(
                message: AppStrings.failedToLoadIcon,
